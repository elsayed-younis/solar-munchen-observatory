...
const I18N = {
  en: {
    overview: 'Overview', map: 'Spatial Map', analytics: 'Analytics',
    crumb_pre: 'Solar·München / ', brand_sub: 'PV Potential Observatory',
    hero_tag: 'ROOF-BY-ROOF ASSESSMENT',
    hero_h: 'How much sun is<br>sleeping on these roofs?',
    hero_p: 'A spatial photovoltaic audit of every qualified building in Maxvorstadt — geometry from OpenStreetMap, radiation from the EU PVGIS satellite model.',
    ...
    lead: 'Rooftops here hide <em>{gwh} GWh</em> a year — clean power for roughly <em>{homes} homes</em>, computed roof by roof, not guessed.',
    m_eff: 'median output density', m_eff_u: 'kWh per m² of roof',
    m_top: 'top 20% of roofs', m_top_u: 'of all energy',
    m_avg: 'average per roof', m_avg_u: 'kWh / year',
    m_exc: 'excellent roofs', m_exc_u: '> 10 MWh / year',
    ...
    c_type: 'Yield by building type', w_type: 'Real OSM use-class, not roof area — where the harvest sits.',
  ...
  de: {
    overview: 'Übersicht', map: 'Karte', analytics: 'Analytik',
    crumb_pre: 'Solar·München / ', brand_sub: 'PV-Potenzial-Observatorium',
    hero_tag: 'DACH-FÜR-DACH-ANALYSE',
    hero_h: 'Wie viel Sonne schläft<br>auf diesen Dächern?',
    hero_p: 'Ein räumliches Photovoltaik-Audit jedes qualifizierten Gebäudes in der Maxvorstadt — Geometrie aus OpenStreetMap, Strahlung aus dem EU-PVGIS-Satellitenmodell.',
    ...
    lead: 'Die Dächer hier verbergen <em>{gwh} GWh</em> pro Jahr — saubere Energie für rund <em>{homes} Haushalte</em>, Dach für Dach berechnet, nicht geraten.',
    m_eff: 'mittlere Ertragsdichte', m_eff_u: 'kWh pro m² Dach',
    m_top: 'oberste 20% der Dächer', m_top_u: 'der gesamten Energie',
    m_avg: 'Durchschnitt pro Dach', m_avg_u: 'kWh / Jahr',
    m_exc: 'exzellente Dächer', m_exc_u: '> 10 MWh / Jahr',
    ...
    c_type: 'Ertrag nach Gebäudetyp', w_type: 'Echte OSM-Nutzungsklasse, nicht Dachfläche — wo der Ertrag liegt.',
...
$('#themeBtn').onclick = () => {
  ...
  if (window._map) setBasemap();
  if (window._charts) restyleCharts();
};

function setLang(l) {
  LANG = l;
  localStorage.setItem('solar-lang', l);
...
function applyLang() {
  ...
  $$('.nav button').forEach(b => b.childNodes[1].nodeValue = ' ' + T(b.dataset.go));
  $('#heroTag').textContent = T('hero_tag');
  $('#heroH').innerHTML = T('hero_h');
  $('#heroP').textContent = T('hero_p');
  $('#heroU').textContent = T('hero_u');
  ...
  $('#slLabel').childNodes[0].nodeValue = T('sl_label') + ' ';
  $('#lgTitle').textContent = T('lg_title');
...
function countUp(el) {
  ...
  (function tick(now) {
    ...
    if (p < 1) requestAnimationFrame(tick); else el.textContent = fmt(target) + suf;
  })(t0);
}
function renderKpis() {
...
function buildCharts() {
  ...
  if (D.by_type.length) {
    ...
    Plotly.newPlot('pType', [{
      x: bt.map(b => b.output), y: bt.map(b => b.type), type: 'bar', orientation: 'h', showlegend: false,
  ...
  Plotly.newPlot('pDist', [{
    ...
    annotations: [
      { x: D.median_out, y: 1, yref: 'paper', text: 'median', showarrow: false, yanchor: 'bottom', font: { size: 11, color: '#f5a623' } },
      { x: D.excellent_thr, y: 1, yref: 'paper', text: 'excellent', showarrow: false, yanchor: 'bottom', font: { size: 11, color: '#e8632a' } }
    ]
  }, CFG);

  Plotly.newPlot('pMarg', [{
    x: D.marginal.x, y: D.marginal.y, type: 'scatter', mode: 'lines', fill: 'tozeroy', showlegend: false,
    ...
    annotations: [{
      ...
      font: { size: 12, color: '#e8632a', family: 'Space Grotesk' }
    }]
  }, CFG);
...
function restyleCharts() {
  ['pLor', 'pType', 'pSpa', 'pDist', 'pMarg', 'pTop'].forEach(id => {
    const e = document.getElementById(id);
    if (e && e.data) Plotly.relayout(e, base());
  });
}
function resizeAll() {
  ['pLor', 'pType', 'pSpa', 'pDist', 'pMarg', 'pTop'].forEach(id => {
...
function observeResize() {
  ...
  resizeObserver = new ResizeObserver(() => {
    if ($('.view[data-view=analytics]').classList.contains('active')) resizeAll();
  });
  $$('.cell .plot').forEach(p => resizeObserver.observe(p));
}
...
function renderLegend() {
  $('#legend').innerHTML =
    '<div style="font-size:10px;letter-spacing:1px;text-transform:uppercase;color:var(--muted);margin-bottom:6px">' + T('lg_title') + '</div>' +
...
const TILE_SOURCES = [
  ...
  { url: 'https://server.arcgisonline.com/ArcGIS/rest/services/World_Street_Map/MapServer/tile/{z}/{y}/{x}',
    attr: '&copy; Esri, HERE, Garmin, USGS' },
  { url: 'https://maps.wikimedia.org/osm-intl/{z}/{x}/{y}.png',
    attr: '&copy; Wikimedia maps' }
];
let tilesLayer = null, tileIndex = 0, tileErrors = 0;
function spawnTiles() {
  const source = TILE_SOURCES[tileIndex];
  tilesLayer = L.tileLayer(source.url, { maxZoom: 19, attribution: source.attr });
  tilesLayer.on('tileerror', () => {
    tileErrors++;
    if (tileErrors > 8 && tileIndex < TILE_SOURCES.length - 1) {
      tileIndex++; tileErrors = 0;
      if (window._map && tilesLayer) {
        window._map.removeLayer(tilesLayer);
        spawnTiles();
        tilesLayer.addTo(window._map);
      }
    }
  });
  return tilesLayer;
...
function setBasemap() {
  ...
  if (!tilesLayer) spawnTiles();
  if (!window._map.hasLayer(tilesLayer)) tilesLayer.addTo(window._map);
}
...
function initMap() {
  ...
  const sliderMax = Math.max(...window.BUILDINGS.features.map(f => f.properties.o));
  window._geoLayer = L.geoJSON(window.BUILDINGS, {
    style: f => {
      const c = D.cat_colors[f.properties.c];
      return { color: c, weight: 0.6, fillColor: c, fillOpacity: 0.55 };
    },
    onEachFeature: (f, layer) => {
      layer.bindTooltip(`<b>${catName(f.properties.c)}</b><br>${fmt(f.properties.o)} kWh/yr`, { sticky: true, direction: 'top' });
      layer.on('mouseover', e => e.target.setStyle({ weight: 2.2, fillOpacity: 0.85 }));
      layer.on('mouseout', e => window._geoLayer.resetStyle(e.target));
    }
  }).addTo(m);
  try { m.fitBounds(window._geoLayer.getBounds(), { padding: [30, 30] }); } catch (e) {}
  const sl = $('#slider');
  sl.max = sliderMax; sl.value = 0; sl.step = Math.max(50, Math.round(sliderMax / 200));
  sl.oninput = () => applySlider(+sl.value);
  $('#baseBtn').onclick = () => {
    document.documentElement.dataset.theme = document.documentElement.dataset.theme === 'light' ? 'dark' : 'light';
    localStorage.setItem('solar-theme', document.documentElement.dataset.theme);
    setBasemap();
    if (window._charts) restyleCharts();
  };
  $('#fitBtn').onclick = () => m.fitBounds(window._geoLayer.getBounds(), { padding: [30, 30] });
  applySlider(0);
}
function applySlider(min) {
  $('#slVal').textContent = fmt(min) + ' kWh';
  let n = 0, sum = 0;
  window._geoLayer.eachLayer(l => {
    const o = l.feature.properties.o, on = o >= min;
    l.setStyle({ fillOpacity: on ? 0.55 : 0.04, opacity: on ? 1 : 0.12, weight: on ? 0.6 : 0.2 });
    if (on) { n++; sum += o; }
  });
...
$('#mEffV').textContent = D.glance.eff;
$('#mTopV').textContent = D.glance.top20 + '%';
$('#mAvgV').textContent = D.glance.avg;
$('#mExcV').textContent = D.glance.exc + '%';
setLang(LANG);
showView('overview');
